...
class Subject(object):
    ...
    def read_twix_files(self):
        ...
        self.dict_dis = io_utils.read_dis_twix( # gas exchange twix files (xe_radial_Dixon)
            io_utils.get_dis_twix_files(str(self.config.data_dir))
        )
        try:
            self.dict_dyn = io_utils.read_dyn_twix( # dynamic spectroscopy twix files xe_calibraton (xe_calibration)
                io_utils.get_dyn_twix_files(str(self.config.data_dir))
            )
        except ValueError:
            logging.info("No dynamic spectroscopy twix file found")
        if self.config.recon.recon_proton: # 1H_radial file
    ...
    def read_mat_file(self):
        ...
        logging.info("[Haad] About to set image_gas_cor")
    ...
    def segmentation(self):
        ...
        if self.config.segmentation_key == constants.SegmentationKey.CNN_VENT.value:
            ...
            This might be where we can use split masks
            """
            try: 
    ...
    def registration(self):
        ...
        """
        """
        Haad: This might be where we can use split masks to compute VDP for all five lobar regions
        """
